...
class Window(tk.Tk):
    ...
    def receive_chat(self):
        ...
        model = ollama_chat.Model(self.model_var.get(), MASTER_DIRECTORY)
        if model.image_analysis and self.selected_file:
            if not self.selected_file.endswith(('.jpg', '.jpeg', '.png')):
                return
            query = ollama_chat.Chat(user_message, image_path=self.selected_file, save_name=save_name)
        elif self.selected_file:
            if self.selected_file.endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.svg', '.ico', '.tiff', '.raw', '.pdf')):
                return
...
def launch(exe_location=None):
    global MASTER_DIRECTORY
    MASTER_DIRECTORY = exe_location if exe_location else None    
    Window().run()
