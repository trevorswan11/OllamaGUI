...
class Model:
    def __init__(self, model: str, save_directory: str = None):
        ...
        if save_directory is None:
            ...
            self.save_directory = f"{save_directory}\\logs\\{self.model.replace(':', '-')}_chat_logs"
    ...
    def chat(self, chat: Chat):
        ...
        chat.save_name = chat.save_name.replace(' ', '-') if chat.save_name else self.model.replace(':', '-')
        with open(
                f"{self.save_directory}/{curr_date}_{chat.save_name + '-chat'}_{curr_time}.md", 'w'
            ) as f:
            f.write(f"# Prompt:\n\n{chat.message}\n\n")
